...
class TestStepMethods:  # yield test doesn't work subclassing object
    ...
    def check_trace(self, step_method):
        ...
        with Model() as model:
            ...
            if step_method.__name__ == "NUTS":
                ...
                trace = sample(
                    0, tune=n_steps, discard_tuned_samples=False, step=step, chains=1
                )
            else:
                trace = sample(
                    0,
    ...
    def check_stat(self, check, trace, name):
        for (var, stat, value, bound) in check:
            ...
            for i in range(len(s)):
                assert np.abs(s[i]-(value[i] if isinstance(value, (list, np.ndarray)) else value)) < (bound[i] if isinstance(bound,(list, np.ndarray)) else bound)
    ...
    def test_step_categorical(self):
        ...
        for step in steps:
            trace = sample(8000, tune=0, step=step, start=start, model=model, chains=1)
            self.check_stat(check, trace, step.__class__.__name__)
